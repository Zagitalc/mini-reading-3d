...
export async function fetchTraffic(url:string,token?:string){const r=await fetch(url,{headers:token?{Authorization:`Bearer ${token}`}:{},redirect:'error',signal:AbortSignal.timeout(12000)});if(!r.ok)throw Error(`Traffic provider returned HTTP ${r.status}`);return parseTraffic(await r.json());}

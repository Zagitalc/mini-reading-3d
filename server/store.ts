import{DatabaseSync}from'node:sqlite';import type{RoadEvent}from'../shared/types';import{eventIsVisible}from'../shared/road-events';
export class RoadEventStore{
 db:DatabaseSync;lastCleanup=0;
 ...
 active(now=Date.now()):RoadEvent[]{return this.db.prepare(`SELECT body FROM road_events WHERE status IN ('active','planned') AND (end_at IS NULL OR julianday(end_at)>=julianday(?)) ORDER BY observed_at DESC`).all(new Date(now).toISOString()).map(r=>JSON.parse(String(r.body))).filter(e=>eventIsVisible(e,now));}
 ...
 record(id:string){this.db.prepare('INSERT OR IGNORE INTO sns_messages VALUES(?,?)').run(id,Date.now());if(Date.now()-this.lastCleanup>3600000){this.db.prepare('DELETE FROM sns_messages WHERE received_at < ?').run(Date.now()-7*86400000);this.lastCleanup=Date.now();}}
 close(){this.db.close();}
}

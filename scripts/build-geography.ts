...
const dest='public/data';await mkdir(dest,{recursive:true});
...
const limitOf=(s:string)=>s.includes('national')||s==='GB:nsl_single'||s==='GB:nsl_dual'?'NSL':s.match(/^(20|30)(?: mph)?$/)?.[1];
for(const f of input.features){const p=f.properties??{},g=f.geometry,id=String(p.osm_id);if(!g)continue;
 if(p.building&&(g.type==='Polygon'||g.type==='MultiPolygon')){
  ...
  for(let n=0;n<polys.length;n++){const rings=polys[n] as LngLat[][];if(!rings[0]||rings[0].length<4)continue;const box=boundsOf(rings[0]);if(!intersects(box,BOUNDS))continue;const center:LngLat=[(box[0]+box[2])/2,(box[1]+box[3])/2];
   ...
   const named=LANDMARKS.find(l=>l.footprintIds.includes(id));
   ...
   map.push({type:'Feature',geometry:{type:'Polygon',coordinates:rings},properties:{...p,kind:'building',height:b.height,colour:b.colour,id:b.id,landmark:b.landmark??''}});
  }continue;
 }
 if(g.type==='Point'){
  ...
  const limit=limitOf(String(p.maxspeed??''))??String(p.traffic_sign??'').match(/274\[(20|30)\]|670\[(20|30)\]/)?.slice(1).find(Boolean)??(String(p.traffic_sign??'').includes('671')?'NSL':undefined);

...
export function nearestOnLine(p:LngLat,line:LngLat[]){let best={position:line[0]??p,distance:Infinity,along:0,segment:0};let run=0;const q=toLocal(p);for(let i=1;i<line.length;i++){const a=toLocal(line[i-1]),b=toLocal(line[i]);const dx=b[0]-a[0],dy=b[1]-a[1],len=Math.hypot(dx,dy);const t=len?Math.max(0,Math.min(1,((q[0]-a[0])*dx+(q[1]-a[1])*dy)/(len*len))):0;const pos=interpolate(line[i-1],line[i],t),d=distance(p,pos);if(d<best.distance)best={position:pos,distance:d,along:run+len*t,segment:i-1};run+=len;}return best;}
